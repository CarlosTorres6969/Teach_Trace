...
export class SeedService implements OnApplicationBootstrap {
  ...
  async onApplicationBootstrap() {
    ...
    // ─── Conversaciones de demostración (HU-35) ───────────────────────────────
    await this.ensureDemoConversations(student, teacher);

    // ─── Notificaciones de demostración (HU-34) ───────────────────────────────
    await this.ensureDemoNotifications(student, teacher);
  }
  ...
  private async ensureDemoConversations(student: User, teacher: User) {
    const existing = await this.conversations.count({ where: { student: { id: student.id } } });
    if (existing > 0) return;

    // Conversación 1 — resuelta
    const conv1 = await this.conversations.save(
      this.conversations.create({
        student,
        teacher,
        submission: null,
        subject: 'Consulta sobre criterio de Argumentación',
        status: ConversationStatus.RESOLVED,
      }),
    );
    await this.messages.save([
      this.messages.create({ conversation: conv1, sender: student, body: 'Hola profesor, tenía una duda sobre el nivel 2 que obtuve en Argumentación. ¿Qué aspectos específicos podría mejorar para la próxima entrega?' }),
      this.messages.create({ conversation: conv1, sender: teacher, body: 'Hola Diego. Tu argumento principal está bien planteado, pero le falta sustento con fuentes académicas. Te recomiendo usar al menos 2 referencias secundarias para reforzar cada punto central.' }),
      this.messages.create({ conversation: conv1, sender: student, body: 'Entendido, muchas gracias. Voy a trabajar en eso para el siguiente ensayo.' }),
      this.messages.create({ conversation: conv1, sender: teacher, body: '¡Perfecto! Si tienes más dudas no dudes en escribir. ¡Éxito!' }),
    ]);

    // Conversación 2 — abierta
    const conv2 = await this.conversations.save(
      this.conversations.create({
        student,
        teacher,
        submission: null,
        subject: 'Duda sobre uso de IA en próxima actividad',
        status: ConversationStatus.OPEN,
      }),
    );
    await this.messages.save([
      this.messages.create({ conversation: conv2, sender: student, body: 'Profesor, para el debate de autos autónomos, ¿podemos usar IA para generar los argumentos iniciales o solo para organizar ideas?' }),
      this.messages.create({ conversation: conv2, sender: teacher, body: 'Puedes usarla para organizar y contrastar, pero los argumentos deben ser elaborados por ti. Recuerda declararlo correctamente en la plataforma.' }),
    ]);
  }

  private async ensureDemoNotifications(student: User, teacher: User) {
    ...
    const msgNotif = this.notifications.create({
      user: student,
      type: NotificationType.MESSAGE_RECEIVED,
      title: 'Nuevo mensaje de Carlos Torres',
      message: 'Asunto: "Consulta sobre criterio de Argumentación"',
      read: true,
      activityId: null,
      conversationId: null,
    });
    await this.notifications.save(msgNotif);
  }
